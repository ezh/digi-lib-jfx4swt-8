...
class View() extends JFXView() {
  /**
   * Prepares to painting by locking native surface.
   *
   * Called on the render thread.
   */
  protected def _begin(pointer: Long): Unit = {}
  ...
  protected def _scheduleRepaint(pointer: Long): Unit = {}
  protected def _setParent(pointer: Long, parentPtr: Long) {}
  protected def _uploadPixels(pointer: Long, pixels: JFXPixels) {}
}
