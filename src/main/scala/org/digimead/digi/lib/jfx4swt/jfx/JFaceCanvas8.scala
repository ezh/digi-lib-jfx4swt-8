...
class JFaceCanvas8(host: WeakReference[FXHost]) extends JFaceCanvas(host) {
  ...
  protected def closeExec(onDispose: JFaceCanvas ⇒ _) = JFX.execAsync {
    lock.synchronized {
      ...
      host.get.foreach(_.asInstanceOf[FXHost8].dispose())
      impl_peer = null
      JFX.execAsync {
  ...
  protected def openExec(scene: Scene, onReady: JFaceCanvas ⇒ _) = JFX.execAsync {
    lock.synchronized {
      host.get.foreach(_.asInstanceOf[FXHost8].setScene(scene))
      if (getScene() == null) {
        setScene(scene)
        host.get.foreach(host ⇒ { impl_peer = Toolkit.getToolkit.createTKEmbeddedStage(host, AccessController.getContext()) })
      } else {
        setScene(scene)
      }
      show()
      JFX.execAsync {
        ...
        lock.synchronized { host.get.foreach(_.sceneNeedsRepaint()) }
        try onReady(this)
